...
- Real-time write to Google Sheets
...
- Google Sheets with the column schema below
...
| Google Sheets OAuth | Google OAuth2 | Write to Google Sheets node |
...
Create a sheet named `Pipeline` with these columns:
...
Set `GOOGLE_SHEET_ID` in your environment variables to the sheet's ID (found in the URL).
